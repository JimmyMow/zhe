...
zhe.StrongSocket = function(url, version, settings) {

  var now = function() {
    return new Date().getTime();
  };

  var settings = $.extend(true, {}, zhe.StrongSocket.defaults, settings);
  var url = url;
  var version = version;
  ...
  var connect = function() {
    ...
    var fullUrl = "wss://" + window.location.hostname + ":4242/lobby/socket";
  ...
  var handle = function(m) {
    ...
    switch (m.t || false) {
      case false:
        break;
      case 'resync':
        if (!options.prodPipe) zhe.reload();
        break;
      case 'ack':
        ackableMessages = [];
        break;
  ...
  var baseUrl = function() {
    ...
    var url = zhe.storage.get(key);
    if (!url) {
      url = urls[0];
      zhe.storage.set(key, url);
    } else if (tryOtherUrl) {
      tryOtherUrl = false;
...
zhe.StrongSocket.sri = Math.random().toString(36).substring(2);
zhe.StrongSocket.available = window.WebSocket || window.MozWebSocket;
zhe.StrongSocket.defaults = {
  events: {

  },
  params: {
    sri: zhe.StrongSocket.sri
  },
  options: {
    name: "unnamed",
